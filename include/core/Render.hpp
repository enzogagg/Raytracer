...
public:
   ...
   *
   * @note the width parameters and the height one are not the same depending on
   * the mode
   * @note the fast render will have a smallest size of screen compare to the
   * normal render
   */
  Color renderPixel(int x, int y, int maxWidth, int maxHeight);

  Color ComputeSpecularLight(Math::Vector &lightSource, Math::Vector normal,
                             const Camera &cam,
  ...
  Color ComputePixelColor(std::shared_ptr<IPrimitive> closest_object,
   ...
   * @param hit_point The point hit by the current ray in the closest shape of
   * the current one
   * @param closest_object a pointer to the closest shape of the current one
   * @return true if it is, false if not
   */
  bool isInShadowOfShape(Math::Point &hit_point, Math::Vector &lightSource,
                         double distToLight);
   ...
   */
  void sendRay(Ray &ray, Color &pixel, Camera &cam, int depthReflexion,
