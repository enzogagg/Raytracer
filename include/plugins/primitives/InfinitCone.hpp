...
/**
 * @file InfinitCone.cpp
 * @brief InfinitCone class implementation.
 * @details This file contains the implementation of the InfinitCone class,
 *      which is a plugin for the raytracer project.
 * @note The InfinitCone class represents a InfinitCone in 3D space.
 */
 ...
 */
class InfinitCone : public IPrimitive {
    public:
        /**
...
extern "C" {
    ...
    /**
     * @brief Create a InfinitCone object.
     * @details This function creates a new InfinitCone object.
     * @param setting The settings for the InfinitCone object.
     * @return A pointer to the created InfinitCone object.
     */
    std::shared_ptr<IPrimitive> createPrimitive(const libconfig::Setting &setting)
    {
        try {
            if (!setting.exists("x") || !setting.exists("y") ||
                ...
                throw std::runtime_error("Missing required fields (x, y, z, height, angle, color) in InfinitCone settings.");
            if (!setting["color"].isGroup())
                throw std::runtime_error("InfinitCone: color is not a group");
            ...
            if (!heightSetting.exists("x") || !heightSetting.exists("y") ||
                ...
                throw std::runtime_error("Invalid or missing height fields (x, y, z) in InfinitCone settings.");
            ...
            if (!colorSetting.exists("r") || !colorSetting.exists("g") ||
                ...
                throw std::runtime_error("Invalid or missing color fields (r, g, b) in InfinitCone settings.");
            ...
            auto getFloatOrInt = [](const libconfig::Setting &s) -> float {
                ...
                throw std::runtime_error("InfinitCone: value is not a number");
            };
            ...
            if (material == "Default") {
                ...
                throw std::runtime_error("InfinitCone: Unknown material type");
            }
            return std::make_shared<InfinitCone>(position, height, angle, color, mat);
    ...
    void registerPlugin()
    {
        PluginsManager::registerPrimitiveFactory("infinite_cone", &createPrimitive);
    }
};
